...
- Daloopa / J-Quants sync / 新provider には踏み出さない。
...
6. `--skip-brief` が無ければ:
   ...
   - `write_llm_handoff(...)`
   - LLM APIは呼ばない。
7. 最後に `outputs/daily_update_<asof>.md` と `outputs/daily_update_<asof>.json` を生成する。
...
features → research_queue → llm-brief まで更新して。
...
4. success path:
   - sync/features/research/brief の各結果をまとめる
   - `outputs/daily_update_<asof>.md/json` を書く
5. `--skip-sync` など skip オプションが効く。
6. `--max-brief-items <= 0` を拒否。
7. 禁止語ガード:
...
python3 -m radar daily-update --asof 2026-06-20 --edinet-codes-file data/metadata/edinetdb_company_codes_20260619.txt --edinet-offset 1200 --edinet-limit 1 --years 5 --period annual --max-brief-items 5 --dry-run
```
