# SYNC_PHASE_B_PLAN — 最小 sync(Phase B)実装計画(実装前)
...
> **本書はコードを含まない。** 実 API を叩かない・raw 保存しない・実装しない。
...
- **retention/purge**: LICENSE_MATRIX **E3 で確定した規則**に従う(例: 一時キャッシュとして N 日 → 再取得 or purge)。
  確定するまで sync は NO-GO。purge は再現情報(provenance + 必要なら derived)確保後のみ(DATA_LAYER §7)。
...
- `source_url`(キー秘匿値を含めない) / `license_scope`(=personal) / `schema_version` / `fetch_id`
...
## 11. GO / NO-GO ゲート
- **現状: NO-GO**(LICENSE_MATRIX 未確認のため)。
- **GO 条件(すべて満たす)**:
...
- これらが揃ったら **EDINET DB companies の1ページ取得 → raw+provenance 保存 → hash 再現** を最小実装。
