...
| plan(本人契約) | **未確認**(プラン名) | **未確認**(本人契約プラン。公開docs上はFree=100/日、Pro=1,000/日) |
| 規約URL | **未確認**(要記入) | `edinetdb.com/legal/terms`(AI下書き確認・本人FACT化待ち) |
| 確認日 | 未記入 | AI下書き 2026-06-18 / 本人確認待ち |
| APIプログラム利用 | 未確認 | 可(REST/MCP提供。APIキー条件あり) |
| raw ローカルキャッシュ可否 | **未確認(致命)** | 条件付き可(性能目的の一時キャッシュ。定期再取得条件あり・具体期間は未確認) |
| 保存期間 / retention / purge義務 | 未確認 | 具体期間は未確認(定期再取得条件のみ確認) |
...
| rate limit | 未確認 | Free 100/日・Pro 1,000/日・Business 10,000/日(公開docs確認。本人契約プランは未確認) |
...
| 未確認事項(列挙) | ToS全般 | 本人契約プラン、具体retention日数、任意の第三者LLMへのraw投入・AI側保持/学習、B2B/第三者システム統合の個別契約条件 |
...
**未確認(FACT扱い禁止)**: J-Quants 有料 ToS 全般、EDINET DB の本人契約プラン名・具体retention日数・任意の第三者LLMへのraw投入/AI側保持/学習、FSA EDINET の具体ライセンス版(Public Data License 1.0 か否か)。
...
## 判定ゲート
- **致命**: J-Quantsは「raw保存」「**第三者LLM入力**」「再配布」が許容と確認できるまで sync は NO-GO。EDINET DB は raw一時キャッシュ/再配布禁止は確認済みだが、第三者LLM raw投入とAI側保持/学習が未確認のため、公式MCP以外のLLM投入は NO-GO。
...
2. **B(sync=raw保存)** ← 追加で **J2/J3 と E2/E3 が「可」**(保持/purge 条件を実装に反映)。
   - AI下書き判定: **J2=未確認 / J3=条件あり / E2=条件付きYes / E3=未確認 → NO-GO/判定不能**。J-Quantsは解約/退会・上位プラン変更後の取得済みデータ削除義務があるため、retention/purge 実装が必須。EDINET DBは一時キャッシュ可だが具体retention日数は未確認。
