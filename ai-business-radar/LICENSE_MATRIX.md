...
- **EDINET DB**(運営 Cabocia株式会社)。**両ドメインとも公式**:
  ...
  - FSA EDINET の有報を日次同期、JP GAAP/IFRS/US GAAP を正規化。**無料 100/日・Pro 1,000/日(公開docs確認・本人契約プランは要確認)**。
  - **canonical/allowed domains**: REST=`edinetdb.jp/v1` / MCP=`edinetdb.jp/mcp`。規約・英語docs=`edinetdb.com`。
...
| domain | api.jquants.com 等(要確認) | REST/MCP=`edinetdb.jp` / 規約・英語docs=`edinetdb.com`(両公式) |
| plan(本人契約) | **未確認**(プラン名) | Pro(本人申告。公開docs上はPro=1,000/日) |
...
| attribution / source表記 | 未確認 | 公開サービスで "Powered by EDINET DB" 等が必要 |
| 禁止事項 | 未確認 | スクレイピング/APIキー共有/一括再配布/wrapper API/過負荷/AI学習用Web収集は禁止。AI所見の不正確性リスク明記 |
| 未確認事項(列挙) | ToS全般 | 具体retention日数、任意の第三者LLMへのraw投入・AI側保持/学習、B2B/第三者システム統合の個別契約条件 |
...
| J2 | 取得データを**ローカルに保存(raw キャッシュ)**してよいか(=sync の前提) | 本人判断GO(2026-06-20・個人内/非公開/再配布なし)。原本転記は未完 | <https://jpx-jquants.com/ja/help/usage> — 利用目的・ライセンス | 本人判断:2026-06-20 / 原本転記待ち | 自身の投資分析/ポートフォリオ管理は私的利用とされる。rawキャッシュ保存の明示条件は公開Helpだけでは特定不能のため、公開・第三者提供・wrapper用途には使わない。 |
| J3 | 保存の**保持期間/purge 義務**はあるか(あれば期間) | 本人判断GO(2026-06-20・手動purge/解約時削除運用)。契約中retention原本転記は未完 | <https://jpx-jquants.com/ja/help/usage> — サブスクリプションのキャンセルまたは退会後 / プラン変更後 | 本人判断:2026-06-20 / 原本転記待ち | 解約/退会後は取得済みデータ削除、上位プランで取得したデータはプラン変更後に削除、と説明。日常運用は raw非公開・手動purge 前提。 |
...
| J5 | 取得データを**第三者(Claude等のLLM)に入力**してよいか【致命】 | derived/evidence/brief は本人判断GO(2026-06-20・個人内分析)。raw本文はNO-GO。原本転記は未完 | <https://jpx-jquants.com/ja/help/usage> — 私的利用とは / アプリ公開・配布 | 本人判断:2026-06-20 / 原本転記待ち | Helpは「本データを第三者が閲覧できる状態」は私的利用ではない、アプリ運営者サーバ蓄積・中継は禁止と説明。運用上は raw本文を渡さず、ローカルderived/evidence/briefだけを個人内分析として扱う。 |
...
### B. EDINET DB(Cabocia・REST=edinetdb.jp/v1 / MCP=edinetdb.jp/mcp)
原本: 規約 URL = `edinetdb.com/legal/terms`(AI下書き + 本人確認 2026-06-18) / 契約プラン = Pro(本人申告。公開docs上はPro 1,000/day)
...
| E2 | 取得データを**ローカルに保存(raw キャッシュ)**してよいか・「一時キャッシュ」の定義/期間 | 条件付きYes(性能目的の一時キャッシュ可。本人確認済み) | <https://edinetdb.com/legal/terms> — 5-1. Permitted Use | 2026-06-18(本人確認) | APIレスポンスを性能目的で一時キャッシュすることは可。ただし定期的にAPIから最新データを再取得する条件あり。具体的な保存期間は未記載。 |
| E3 | **保持期間/purge 義務** | 条件付きYes(具体日数なし。定期再取得 + 手動purgeで運用) | <https://edinetdb.com/legal/terms> — 5-1. Permitted Use / 12. Distribution of IR-Related PDFs | 2026-06-18(本人確認) | 一時キャッシュは定期再取得が条件。ユーザー側rawキャッシュの保持期間/purge義務の具体日数は未記載。IR PDFの3-5年アーカイブ方針はサービス側配布物の説明で、ユーザー保存期間ではない。 |
...
| E7 | **再配布**(一括/wrapper API)は禁止か | Yes(一括再配布・wrapper/proxy APIは禁止。B2B/第三者システム統合は別契約) | <https://edinetdb.com/legal/terms> — Point Summary / 5-2. Prohibited Use / 5-3. Commercial Data Provision to Third Parties | 2026-06-18(本人確認) | API/MCP取得データの全部または相当部分を第三者へ一括提供・再配布すること、実質同等API(wrapper/proxy)を提供することは禁止。第三者システム/DBへの直接保存・統合は別契約が必要。 |
| E8 | **契約プランの上限**(Pro=1,000/日 で合っているか)・課金 | 条件付き確認(公開docs: Free 100/day, Pro 1,000/day, Business 10,000/day。本人契約=Pro申告) | <https://edinetdb.com/docs/api> — Public REST API / Rate Limits; <https://edinetdb.com/developers> — Pricing Plans; <https://edinetdb.com/legal/terms> — 6. Plans and Billing | 2026-06-18(本人確認) | API docsにRate Limits per accountとしてAnonymous/Free 100/day、Pro 1,000/day、Business 10,000/day。Paid plansは月額サブスクでStripe決済。 |
